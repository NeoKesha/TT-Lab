...
namespace TT_Lab.Project
{
    ...
    public class Project : IProject
    {
        ...
        public void UnpackAssetsPS2()
        {
            ...
            var packagesFolder = Folder.CreatePS2Folder("Packages");
            var blendSkinsFolder = Folder.CreatePS2Folder("Blend Skins", packagesFolder);
            var skinsFolder = Folder.CreatePS2Folder("Skins", packagesFolder);
            var modelsFolder = Folder.CreatePS2Folder("Models", packagesFolder);
            var rigidModelsFolder = Folder.CreatePS2Folder("Rigid models", packagesFolder);
            var meshesFolder = Folder.CreatePS2Folder("Meshes", packagesFolder);
            var materialsFolder = Folder.CreatePS2Folder("Materials", packagesFolder);
            var lodsFolder = Folder.CreatePS2Folder("LODs", packagesFolder);
            var skydomesFolder = Folder.CreatePS2Folder("Skydomes", packagesFolder);
            var texturesFolder = Folder.CreatePS2Folder("Textures", packagesFolder);
            var animationsFolder = Folder.CreatePS2Folder("Animations", packagesFolder);
            var codeModelsFolder = Folder.CreatePS2Folder("Code models", packagesFolder);
            var gameObjectsFolder = Folder.CreatePS2Folder("Game objects", packagesFolder);
            var scriptsFolder = Folder.CreatePS2Folder("Scripts", packagesFolder);
            var ogisFolder = Folder.CreatePS2Folder("OGIs/Skeletons", packagesFolder);
            var sfxFolder = Folder.CreatePS2Folder("Sound Effects", packagesFolder);
            var enFolder = Folder.CreatePS2Folder("English Sound Effects", packagesFolder);
            var frFolder = Folder.CreatePS2Folder("French Sound Effects", packagesFolder);
            var grFolder = Folder.CreatePS2Folder("German Sound Effects", packagesFolder);
            var itaFolder = Folder.CreatePS2Folder("Italian Sound Effects", packagesFolder);
            var jpnFolder = Folder.CreatePS2Folder("Japanese Sound Effects", packagesFolder);
            var spaFolder = Folder.CreatePS2Folder("Spanish Sound Effects", packagesFolder);
            var chunksFolder = Folder.CreatePS2Folder("Chunks", packagesFolder);
            ...
            assets.Add(packagesFolder.UUID, packagesFolder);
            ...
            foreach (var item in archive.Items)
            {
                ...
                if (isRm2 || isSm2)
                {
                    ...
                    ReadSectionItems<Texture, PS2AnyTexturesSection, PS2AnyTexture>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_TEXTURES_SECTION, texturesFolder);
                    ReadSectionItems<Skydome, PS2AnySkydomesSection, PS2AnySkydome>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_SKYDOMES_SECTION, skydomesFolder);
                    ReadSectionItems<Material, PS2AnyMaterialsSection, PS2AnyMaterial>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_MATERIALS_SECTION, materialsFolder);
                    ReadSectionItems<Model, PS2AnyModelsSection, PS2AnyModel>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_MODELS_SECTION, modelsFolder);
                    ReadSectionItems<RigidModel, PS2AnyRigidModelsSection, PS2AnyRigidModel>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_RIGID_MODELS_SECTION, rigidModelsFolder);
                    ReadSectionItems<Skin, PS2AnySkinsSection, PS2AnySkin>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_SKINS_SECTION, skinsFolder);
                    ReadSectionItems<BlendSkin, PS2AnyBlendSkinsSection, PS2AnyBlendSkin>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_BLEND_SKINS_SECTION, blendSkinsFolder);
                    ReadSectionItems<LodModel, PS2AnyLODsSection, PS2AnyLOD>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_LODS_SECTION, lodsFolder);
                    ReadSectionItems<Mesh, PS2AnyMeshesSection, PS2AnyMesh>
                        (assets, graphics, graphicsCheck, Constants.GRAPHICS_MESHES_SECTION, meshesFolder);
                    // Read code stuff
                    ...
                    if (code != null)
                    {
                        ReadSectionItems<GameObject, PS2AnyGameObjectSection, PS2AnyObject>
                            (assets, code, codeCheck, Constants.CODE_GAME_OBJECTS_SECTION, gameObjectsFolder);
                        ...
                        ReadSectionItems<SoundEffect, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_SOUND_EFFECTS_SECTION, sfxFolder);
                        ReadSectionItems<SoundEffectSP, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_LANG_SPA_SECTION, spaFolder);
                        ReadSectionItems<SoundEffectJP, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_LANG_JPN_SECTION, jpnFolder);
                        ReadSectionItems<SoundEffectIT, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_LANG_ITA_SECTION, itaFolder);
                        ReadSectionItems<SoundEffectGR, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_LANG_GER_SECTION, grFolder);
                        ReadSectionItems<SoundEffectFR, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_LANG_FRE_SECTION, frFolder);
                        ReadSectionItems<SoundEffectEN, PS2AnySoundsSection, PS2AnySound>
                            (assets, code, codeCheck, Constants.CODE_LANG_ENG_SECTION, enFolder);
                    }
                    ...
                    if (isRm2)
                    {
                        if (!isDefault)
                        {
                            ...
                            var colData = new Collision("base", "PS2", collisionData.GetID(), collisionData.GetName(), pathLow, collisionData);
                        ...
                        var partData = new Particles("base", "PS2", particleData.GetID(), particleData.GetName(), pathLow, particleData);
                    ...
                    if (isSm2)
                    {
                        ...
                        var sceneryAsset = new Scenery("base", "PS2", scenery.GetID(), scenery.GetName(), pathLow, scenery);
                        var dynamicSceneryAsset = new DynamicScenery("base", "PS2", dynamicScenery.GetID(), dynamicScenery.GetName(), pathLow, dynamicScenery);
                        var chunkLinksAsset = new ChunkLinks("base", "PS2", chunkLinks.GetID(), chunkLinks.GetName(), pathLow, chunkLinks);
