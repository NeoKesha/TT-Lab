...
{
    /// <summary>
    /// XBOX project class, the root has the reserved extension .xson
    /// </summary>
    public class XBOXProject : IProject
    {
        ...
        public XBOXProject(string name, string path, string discContentPath) : this()
        {
            Name = name;
            Path = path;
        ...
        public void Serialize()
        {
            ...
            using (System.IO.FileStream fs = new System.IO.FileStream(Name + ".xson", System.IO.FileMode.Create, System.IO.FileAccess.Write))
        ...
        public static XBOXProject Deserialize(string projectPath)
        {
            XBOXProject pr;
            ...
            using (System.IO.BinaryReader reader = new System.IO.BinaryReader(fs))
            {
                ...
                pr = JsonConvert.DeserializeObject<XBOXProject>(prText);
            }
        ...
        public void UnpackAssets()
        {
            ...
            foreach (string path in System.IO.Directory.GetFiles(DiscContentPath, "", System.IO.SearchOption.AllDirectories))
            {
                var pathLow = path.ToLower().Substring(DiscContentPath.Length);
                var isRmX = pathLow.EndsWith(".rmx");
                var isSmX = pathLow.EndsWith(".smx");
                var isDefault = pathLow.EndsWith("efault.rmx");
                ...
                var isPtl = pathLow.EndsWith(".ptl");
                ...
                using (System.IO.MemoryStream ms = new System.IO.MemoryStream())
                {
                    ...
                    using (System.IO.BinaryReader reader = new System.IO.BinaryReader(ms))
                    {
                        ...
                        if (isRmX || isSmX)
                        {
                            ...
                            if (isDefault)
                            {
                                chunk = new XBOXDefault();
                            }
                            else if (isRmX)
                            {
                                chunk = new XBOXAnyTwinsanityRMX();
                            }
                            else if (isSmX)
                            {
                                chunk = new XBOXAnyTwinsanitySMX();
                                graphicsSectionID = Constants.SCENERY_GRAPHICS_SECTION;
                            }
                            // Fill chunk data
                            chunk.Read(reader, (Int32)ms.Length);
                            // Read graphics stuff
                            var graphics = chunk.GetItem<XBOXAnyGraphicsSection>(graphicsSectionID);
                            ReadSectionItems<Texture, XBOXAnyTexturesSection, XBOXAnyTexture>
                            ...
                            ReadSectionItems<Model, XBOXAnyModelsSection, XBOXAnyModel>
                            ...
                            ReadSectionItems<Skin, XBOXAnySkinsSection, XBOXAnySkin>
                                (graphics, graphicsCheck, Constants.GRAPHICS_SKINS_SECTION, skinsFolder);
                            ReadSectionItems<BlendSkin, XBOXAnyBlendSkinsSection, XBOXAnyBlendSkin>
                            ...
                            // Read code stuff
                            var code = chunk.GetItem<XBOXAnyCodeSection>(Constants.LEVEL_CODE_SECTION);
                            if (code != null)
                            {
                                ReadSectionItems<GameObject, XBOXAnyGameObjectSection, XBOXAnyObject>
                                ...
                                ReadSectionItems<CodeModel, XBOXAnyCodeModelsSection, XBOXAnyCodeModel>
                                ...
                                var items = code.GetItem<XBOXAnyScriptsSection>(Constants.CODE_SCRIPTS_SECTION);
                                ...
                                ReadSectionItems<SoundEffect, XBOXAnySoundsSection, XBOXAnySound>
                                    (code, codeCheck, Constants.CODE_SOUND_EFFECTS_SECTION, sfxFolder);
                                ReadSectionItems<SoundEffectSP, XBOXAnySoundsSection, XBOXAnySound>
                                    (code, codeCheck, Constants.CODE_LANG_SPA_SECTION, spaFolder);
                                ReadSectionItems<SoundEffectJP, XBOXAnySoundsSection, XBOXAnySound>
                                    (code, codeCheck, Constants.CODE_LANG_JPN_SECTION, jpnFolder);
                                ReadSectionItems<SoundEffectIT, XBOXAnySoundsSection, XBOXAnySound>
                                    (code, codeCheck, Constants.CODE_LANG_ITA_SECTION, itaFolder);
                                ReadSectionItems<SoundEffectGR, XBOXAnySoundsSection, XBOXAnySound>
                                    (code, codeCheck, Constants.CODE_LANG_GER_SECTION, grFolder);
                                ReadSectionItems<SoundEffectFR, XBOXAnySoundsSection, XBOXAnySound>
                                    (code, codeCheck, Constants.CODE_LANG_FRE_SECTION, frFolder);
                                ReadSectionItems<SoundEffectEN, XBOXAnySoundsSection, XBOXAnySound>
