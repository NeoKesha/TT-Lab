...
{
    public class XboxBehaviourCommand : ITwinBehaviourCommand
    {
        public UInt32 Bitfield { get; set; }
        public UInt16 CommandIndex { get; set; }
        public List<UInt32> Arguments { get; set; }
        public AgentLabVersion Version { get => AgentLabVersion.Xbox; }
        ...
        {
            AgentLabDefs defs = XboxBehaviourGraph.GetAgentLabDefs();
        ...
        public void ReadText(String line)
        {
            AgentLabDefs defs = XboxBehaviourGraph.GetAgentLabDefs();
            Arguments.Clear();
            Debug.Assert(line.StartsWith($"@{Version}"), "Trying to parse XBox command as a different version");
        ...
        public static UInt32 GetCommandSize(UInt16 index)
        {
            if (commandSizeMap == null)
            {
                ...
                var commandSizes = XboxBehaviourGraph.GetAgentLabDefs().CommandSizes;
